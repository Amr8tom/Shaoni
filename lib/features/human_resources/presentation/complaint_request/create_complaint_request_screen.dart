...
import 'package:shaoni/features/human_resources/presentation/complaint_request/widgets/complaint_request_data_widget.dart';
...
class CreateComplaintRequestScreen extends StatelessWidget {
  const CreateComplaintRequestScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => serviceLocator<ComplaintRequestCubit>(),
      child: Scaffold(
        appBar: DAppBar(
          title: S.current.complaintRequest,
          showMenu: false,
          showBackArrow: true,
        ),
        ...
        body: Builder(
          builder: (context) {
            final controller = context.read<ComplaintRequestCubit>();
            return BlocConsumer<ComplaintRequestCubit, ComplaintRequestState>(
              listener: (context, state) {
                if (state.isError ||
                    state.isCreateRequestError ||
                    state.isTypesError ||
                    state.isReasonsError) {
                    ...
                    child: Stack(
                      children: [
                        ...
                        SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const ComplaintRequestDataWidget(),
                              const Sizer(height: 35),
                        ...
                        state.isCreateRequestLoading
                            ...
                            : CreateDeleteButtons(
                                deleteTab: () =>
                                    controller.deleteComplaintRequest(),
                                createTab: () {
                                  if (controller.requestFormKey.currentState!
                                      .validate()) {
                                    controller.createComplaintRequest();
                                  }
                                },
                              ),
                      ],
