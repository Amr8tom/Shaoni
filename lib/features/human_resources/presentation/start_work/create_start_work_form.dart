...
import 'package:shaoni/features/human_resources/presentation/start_work/widget/start_work_request_data_widget.dart';
...
class CreateStartWorkForm extends StatelessWidget {
  final int? requestId;

  const CreateStartWorkForm({super.key, this.requestId});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => serviceLocator<StartWorkCubit>(),
      child: Scaffold(
        appBar: DAppBar(
          showMenu: false,
          showBackArrow: true,
          title: _isEditMode ? S.current.editRequest : S.current.startWork,
        ),
        extendBodyBehindAppBar: true,
        backgroundColor: ColorRes.grey6,
        body: Builder(
          builder: (context) {
            final controller = context.read<StartWorkCubit>();
            return BlocConsumer<StartWorkCubit, StartWorkState>(
              listener: (context, state) {
                if (state.isCreateError || state.isLookupsError) {
                    ...
                    child: Stack(
                      children: [
                        SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const StartWorkRequestDataWidget(),
                        ...
                        state.isCreateLoading
                            ...
                            : CreateDeleteButtons(
                                deleteTab: () {
                                  controller.deleteStartWorkRequest();
                                },
                                createTab: () {
                                  if (controller.requestFormKey.currentState!
                                      .validate()) {
                                    if (_isEditMode) {
                                      controller.updateStartWorkRequest(
                                          requestId: requestId!);
                                    } else {
                                      controller.createStartWorkRequest();
                                    }
                                  }
                                },
                              ),
