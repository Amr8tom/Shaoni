...
class MyRequestsRepositoryImp extends MyRequestsRepository {
  ...
  Future<Either<Failure, EditResponse>> getSalaryTransferEdit({
    ...
    if (await _networkInfo.isConnected) {
      try {
        final result =
            await _remoteDataSources.getSalaryTransferEdit(params: params);
        return Right(result);
      } on ServerFailure catch (e) {
        return Left(ServerFailure(message: e.message));
      }
  ...
  @override
  Future<Either<Failure, EditResponse>> getTicketBookingEdit({
    required GetTicketBookingEditParams params,
  }) async {
    if (await _networkInfo.isConnected) {
      try {
        final result =
            await _remoteDataSources.getTicketBookingEdit(params: params);
        return Right(result);
      } on ServerFailure catch (e) {
        return Left(ServerFailure(message: e.message));
      }
  ...
  Future<Either<Failure, EditResponse>> getLeaveInterruptionEdit({
    ...
    if (await _networkInfo.isConnected) {
      try {
        final result =
            await _remoteDataSources.getLeaveInterruptionEdit(params: params);
        return Right(result);
      } on ServerFailure catch (e) {
        return Left(ServerFailure(message: e.message));
      }
