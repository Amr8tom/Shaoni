...
import 'package:shaoni/features/human_resoures/presentation/controller/medical_insurance/medical_insurance_cubit.dart';
import 'package:shaoni/features/human_resoures/presentation/medical_insurance/widget/medical_insurance_data_widget.dart';
...
class CreateMedicalInsuranceForm extends StatelessWidget {
  final int? requestId;

  const CreateMedicalInsuranceForm({super.key, this.requestId});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => serviceLocator<MedicalInsuranceCubit>(),
      child: Scaffold(
        appBar: DAppBar(
          showMenu: false,
          showBackArrow: true,
        ...
        body: Builder(
          builder: (context) {
            final controller = context.read<MedicalInsuranceCubit>();
            return BlocConsumer<MedicalInsuranceCubit, MedicalInsuranceState>(
                            ...
                            : CreateDeleteButtons(
                                ...
                                createTab: () {
                                  if (controller.requestFormKey.currentState!
                                      .validate()) {
                                    if (_isEditMode) {
                                      controller.updateMedicalInsurance(
                                          requestId: requestId!);
                                    } else {
                                      controller.createMedicalInsurance();
                                    }
                                  }
                                },
                              ),
