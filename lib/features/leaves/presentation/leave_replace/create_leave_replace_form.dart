...
import 'package:shaoni/features/leaves/presentation/leave_replace/widget/leave_replace_data_widget.dart';
import 'package:shaoni/generated/l10n.dart';

class CreateLeaveReplaceForm extends StatelessWidget {
  final int? requestId;

  const CreateLeaveReplaceForm({super.key, this.requestId});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => serviceLocator<LeaveReplaceCubit>(),
      child: Scaffold(
        appBar: DAppBar(
          showMenu: false,
          showBackArrow: true,
        ...
        body: Builder(
          builder: (context) {
            ...
            return BlocConsumer<LeaveReplaceCubit, LeaveReplaceState>(
  ...
  void _onSubmit(BuildContext context, LeaveReplaceCubit controller) {
    if (!(controller.requestFormKey.currentState?.validate() ?? false)) {
      return;
    }
    if (_isEditMode) {
      controller.updateLeaveReplace(requestId: requestId!);
    } else {
      controller.createLeaveReplace();
    }
  }
}
